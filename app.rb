...
post '/weather' do
  if params[:token] == ENV['SLACK_VERIFICATION_TOKEN']
    ...
    if query == ''
      response = { text: 'You have to enter a location, like `/weather in 1600 Pennsylvania Avenue NW, Washington, DC`. You can enter just a city or zip code, or a full address. You can also specify if you want your results in celsius, like `/weather in new york in celsius`.', response_type: 'ephemeral' }.to_json
    else
